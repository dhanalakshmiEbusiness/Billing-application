...
function generatePolygonPlots(callback){
    ...
    polygonObj.save(function(err,result){
        if(err)
            console.log(err)
        console.log("polygonLoadingBayDetails plots generated")
        /*process.exit(0);*/
        callback();
    })
};
function run(){
    polygonLoadingBayDetails.remove({}, function(err) {
        ...
        console.log("polygonLoadingBayDetails previous data removed")
    });
}
/*generatePolygonPlots();*/
module.exports={
    run:function(callback){
        polygonLoadingBayDetails.remove({}, function(err) {
            generatePolygonPlots(callback);
            console.warn(" polygonLoadingBayDetails previous data removed")
        });
    }
